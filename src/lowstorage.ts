...
const MOD_NAME: string = 'lowstorage';
const DELIMITER: string = '/';
const DIR_PREFIX: string = 'lowstorage';
const COL_SUFFIX: string = '.mpck';
const CHUNK_5MB: number = 5 * 1024 * 1024;
...
class lowstorage {
	 ...
	 * @throws {lowstorageError} If there's an error.
	 */
	async createCollection(colName: string = '', data: any[] = []): Promise<Collection> {
		try {
			_hasColName(colName);
	 ...
	 * @throws {lowstorageError} If there's an error.
	 */
	async collection(colName: string = '', autoCreate: boolean = true): Promise<Collection> {
		try {
			_hasColName(colName);
...
class Collection {
	...
	private _key: string;
	private _packr: Packr;
	constructor(colName: string = '', s3: S3, dirPrefix: string = DIR_PREFIX, chunkSize: number = CHUNK_5MB) {
	...
	async _isSameFile(key: string): Promise<boolean> {
	 ...
	 * @throws {lowstorageError} If there's an error.
	 */
	async insert(doc: Object | Array<Object>): Promise<Object[]> {
		try {
	...
	async renameCollection(newColName: string): Promise<Collection> {
		try {

            ...
            if (!exists) {
                if (typeof schema !== 'undefined' && schema !== null) {
                    try {
                        ...
                        throw new SchemaValidationError(`Schema is invalid: ${error.message} in ${schema} `, lowstorage_ERROR_CODES.SCHEMA_VALIDATION_ERROR);
                    }
                }
...
class Collection {
    ...
    async insert(doc, schema) {
        try {
            ...
            if (!avroType) {
                throw new SchemaValidationError(`Schema is required - Pass a schema to the insert method`, lowstorage_ERROR_CODES.SCHEMA_VALIDATION_ERROR);
            }

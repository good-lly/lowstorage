...
`);var W="return function "+this._getConstructorName()+"(";W+=r.join()+`) {
`+o+"};";var B=new Function(e.join(),W).apply(void 0,n);if(i)return B;var H=this;return B.getType=function(){return H},B.type=H,this._isError&&(_.inherits(B,Error),B.prototype.name=this._getConstructorName()),B.prototype.clone=function(Y){return H.clone(this,Y)},B.prototype.compare=function(Y){return H.compare(this,Y)},B.prototype.isValid=function(Y){return H.isValid(this,Y)},B.prototype.toBuffer=function(){return H.toBuffer(this)},B.prototype.toString=function(){return H.toString(this)},B.prototype.wrap=function(){return H.wrap(this)},B.prototype.wrapped=B.prototype.wrap,B},c.prototype._createChecker=function(){var t=[],i=[],e=this._getConstructorName(),r="return function check"+e+`(v, f, h, p) {
`;if(r+=`  if (
`,r+=`    v === null ||
`,r+=`    typeof v != 'object' ||
...
`);return e+="}",new Function(t.join(),e).apply(void 0,r)},c.prototype._update=function(t,i,e){if(!pe(this,i,!e.ignoreNamespaces))throw new Error(R("no alias found for %s",i.name));var r=this.fields,n=i.fields,o=h.toMap(n,function(we){return we.name}),s=[],f={},y,S,$,L,F,W,B;for(y=0;y<r.length;y++){for($=r[y],F=he($),W=[],S=0;S<F.length;S++)L=F[S],o[L]&&W.push(L);if(W.length>1)throw new Error(R("ambiguous aliasing for %s.%s (%s)",i.name,$.name,W));if(W.length)L=W[0],B={resolver:$.type.createResolver(o[L].type,e),name:"_"+$.name},f[L]?f[L].push(B):f[L]=[B],s.push(B.name);else{if($.defaultValue()===void 0)throw new Error(R("no matching field for default-less %s.%s",i.name,$.name));s.push("undefined")}}var H=-1;for(y=n.length;y&&f[n[--y].name]===void 0;)H=y;var Y=this._getConstructorName(),ne=[Y],ae=[this.recordConstructor],ce="  return function read"+Y+`(t, b) {
`;for(y=0;y<n.length;y++)if(y===H&&(ce+=`  if (!b) {
`),$=i.fields[y],L=$.name,f[L]===void 0)ce+=~H&&y>=H?"    ":"  ",ne.push("r"+y),ae.push($.type),ce+="r"+y+`._skip(t);

...
class lowstorage {
    ...
    async removeCollection(colName = '') {
        try {
            ...
            throw new lowstorageError(`${MODULE_NAME}: Failed to remove collection: ${error.message}`, lowstorage_ERROR_CODES.REMOVE_COLLECTION_ERROR);
        }
    }
    /**
    ...
    async collection(colName = '', schema, autoCreate = true) {
        try {
            ...
            if (error.message.includes('unknown type')) {
                throw new SchemaValidationError(`${MODULE_NAME}: Schema input is invalid: ${error.message}`, lowstorage_ERROR_CODES.SCHEMA_VALIDATION_ERROR);
            }
            throw new lowstorageError(`${MODULE_NAME}: ${error.message}`, lowstorage_ERROR_CODES.COLLECTION_NOT_FOUND);
        }
    }
...
class Collection {
    ...
    async _loadData() {
        try {
            if (this._avroType === null || typeof this._avroType === 'undefined') {
                throw new lowstorageError(`${MODULE_NAME}: Missing type definition. Configure before operations `, lowstorage_ERROR_CODES.SCHEMA_VALIDATION_ERROR);
            }
    ...
    async insert(doc, schema) {
        try {
            ...
            if (typeof doc !== 'object' && !Array.isArray(doc)) {
                throw new DocumentValidationError(`${MODULE_NAME}: Document must be an object or an array`, lowstorage_ERROR_CODES.DOCUMENT_VALIDATION_ERROR);
            }
            ...
            if (!avroType) {
                throw new SchemaValidationError(`${MODULE_NAME}: Schema is required - Pass a schema to the insert method`, lowstorage_ERROR_CODES.SCHEMA_VALIDATION_ERROR);
            }
            this._avroType = avroType;
            const data = await this._loadData();
            for (let item of items) {
                if (typeof item !== 'object' || item === null) {
                    throw new DocumentValidationError(`${MODULE_NAME}: Invalid input: input must be an object or an array of objects`, lowstorage_ERROR_CODES.DOCUMENT_VALIDATION_ERROR);
                }
    ...
    async update(query = {}, update = {}, options = {}) {
        try {
            if (query === undefined || query === null || update === undefined || update === null) {
                throw new lowstorageError(`${MODULE_NAME}: Query and update values are required for update`, lowstorage_ERROR_CODES.MISSING_ARGUMENT);
            }
            if (!this._avroType) {
                throw new SchemaValidationError(`${MODULE_NAME}: Schema is not defined for this collection`, lowstorage_ERROR_CODES.SCHEMA_VALIDATION_ERROR);
            }
            const data = await this._loadData();
            if (data.length === 0)
                return 0;
    ...
    async renameCollection(newColName, newSchema = this._schema) {
        try {
            ...
            throw new lowstorageError(`${MODULE_NAME}: Rename collection failed: ${error.message}`, lowstorage_ERROR_CODES.RENAME_COLLECTION_ERROR);
        }
    }
}
